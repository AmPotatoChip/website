<?php
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null,$contact){
		
		require_once('/var/www/sites/youthleadweb.org/private/dbconn.php');
		require_once('../common/registration_class.php');
		...
		$REGISTER = new REGISTRATION_CLASS();
		if($filesize > 0){	
			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					if($ext==end($file_ext)){
						...
						$newfilename = $contact.".".end($file_ext);
		...
		if(sizeof($errorList) == 0){
			...
			$REGISTER->UpdateProfileImage($newfilename,$contact);
	...
	if($filesize_image > 0){
		$upload_image = uploadImage($filename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH,$contact);
