...
function default_page(){
	...
	SmartyValidate::register_form('main_contact',true);
	if($_GET[contact_id]){
		$data=$CONTACT->getMainContactInfo($_GET[contact_id]);
		$smarty->assign($data);
	}
	...
	$LGCLASS->page_tpl='admin/edit_address_main.tpl'; // This current's page template file.
...
if(!empty($_POST)){
	if(SmartyValidate::is_valid($_POST,'main_contact')){
		SmartyValidate::disconnect();
		$CONTACT->updateContact('main',$_POST);
	}else{
