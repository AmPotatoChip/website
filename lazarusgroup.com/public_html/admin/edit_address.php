...
function default_page(){
	...
	SmartyValidate::register_form('address_form',true);
	if($_GET[address_id] && !$_POST){
		$data=$CONTACT->getAddressById($_GET[address_id]);
		$smarty->assign($data);
	}
	...
	$LGCLASS->page_tpl='admin/edit_address.tpl'; // This current's page template file.
...
if(!empty($_POST)){
	if(SmartyValidate::is_valid($_POST,'address_form')){
		SmartyValidate::disconnect();
		$CONTACT->updateContact('address',$_POST);
	}else{
